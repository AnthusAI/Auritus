"""Fish Speech TTS backend (CC-BY-NC-SA, CUDA required)."""

from __future__ import annotations

import io
import struct
import wave
from typing import Any

from auritus.tts.base import TTSBackend


class FishBackend(TTSBackend):
    """Generate speech with Fish Speech."""

    name = "fish"
    _model = None

    def generate(self, text: str, meta: dict[str, Any]) -> bytes:
        """Generate 24 kHz mono WAV audio with Fish Speech."""
        if not text.strip():
            raise ValueError("Cannot generate audio for empty text")
        if FishBackend._model is None:
            # TODO: Verify the Fish Speech API against the installed release.
            from fish_speech.api import TTS as FishTTS

            FishBackend._model = FishTTS()
        audio = FishBackend._model.generate(text)
        samples = audio.tolist() if hasattr(audio, "tolist") else list(audio)
        return _to_wav(samples, 24000)


def _to_wav(samples: list[Any], sample_rate: int) -> bytes:
    """Convert normalized samples to mono 16-bit WAV bytes."""
    buffer = io.BytesIO()
    with wave.open(buffer, "wb") as handle:
        handle.setnchannels(1)
        handle.setsampwidth(2)
        handle.setframerate(sample_rate)
        frames = struct.pack(
            "<" + "h" * len(samples),
            *[
                max(-32768, min(32767, int(float(sample) * 32767)))
                for sample in samples
            ],
        )
        handle.writeframes(frames)
    return buffer.getvalue()
